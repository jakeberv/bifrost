...
test_that("createSimulationTemplate validates basic inputs and column specifications", {
  ...
  testthat::expect_false(ape::is.rooted(unrooted))
  testthat::expect_error(
    createSimulationTemplate(unrooted, X, formula = "trait_data ~ 1"),
    "baseline_tree must be rooted"
  )

  duplicate_tree <- tr
  duplicate_tree$tip.label[[2L]] <- duplicate_tree$tip.label[[1L]]
  testthat::expect_error(
    createSimulationTemplate(duplicate_tree, X, formula = "trait_data ~ 1"),
    "tree tip labels.*unique"
  )

  duplicate_rows <- rbind(X, X[1L, , drop = FALSE])
  rownames(duplicate_rows) <- c(rownames(X), rownames(X)[[1L]])
  testthat::expect_error(
    createSimulationTemplate(tr, duplicate_rows, formula = "trait_data ~ 1"),
    "trait_data row names.*unique"
  )

  duplicate_columns <- X
  colnames(duplicate_columns) <- c("y", "y", "mass")
  testthat::expect_error(
    createSimulationTemplate(tr, duplicate_columns, formula = "trait_data ~ 1"),
    "trait_data column names.*unique"
  )
...
test_that("createSimulationTemplate supports named formulas with factor predictors", {
  ...
  calibration_tree <- NULL
  paint_subtree <- phytools::paintSubTree
  testthat::local_mocked_bindings(paintSubTree = function(...) {
    calibration_tree <<- paint_subtree(...)
    calibration_tree
  }, .package = "phytools")
  set.seed(61)
  tr <- ape::rtree(18)
  ...
  testthat::expect_s3_class(calibration_tree, "simmap")
  testthat::expect_identical(unique(phytools::getStates(calibration_tree)), "0")
})
