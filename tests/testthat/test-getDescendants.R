...
pkg_get_descendants <- bifrost:::getDescendants
...
test_that("Tip node: no descendants; include.node=TRUE returns the tip", {
  ...
  out1 <- pkg_get_descendants(tr, tip1, include.node = FALSE)
  expect_length(out1, 0)

  out2 <- pkg_get_descendants(tr, tip1, include.node = TRUE)
...
test_that("MRCA(t4,t5) descendants are exactly {t4, t5}", {
  ...
  out <- pkg_get_descendants(tr, n45, include.node = FALSE)
...
test_that("MRCA(t3, MRCA(t4,t5)) descendants include {t3, t4, t5, n45}", {
  ...
  out <- pkg_get_descendants(tr, n345, include.node = FALSE)
...
test_that("Root descendants are all nodes except the root itself", {
  ...
  out <- pkg_get_descendants(tr, root, include.node = FALSE)
...
test_that("Out-of-range node returns empty unless include.node=TRUE", {
  ...
  out1 <- pkg_get_descendants(tr, bogus, include.node = FALSE)
  expect_length(out1, 0)
  ...
  out2 <- pkg_get_descendants(tr, bogus, include.node = TRUE)
