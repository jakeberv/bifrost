...
<br>
```{r troyer_fig1, echo=FALSE, fig.align="center", out.width="60%", fig.alt="Figure 1 from Troyer et al. (2025)."}
...
```
<br>
```{r IC_decay_fig, echo=FALSE, fig.align="center", out.width="80%", fig.alt="Scatter plot showing information-criterion scores across sequentially evaluated sub-models. Accepted shifts are shown as blue points connected by a line, rejected shifts as red crosses, and the baseline IC as a black point. A grey line on a secondary axis shows the change in IC at each step, with labels marking the baseline and lowest accepted IC."}
...
For sensitivity analyses or posterior-style batches of fitted models, `rateMap()`
can summarize branchwise rates across many completed runs on the same tree. The
function accepts `bifrost_search` objects directly, and can also consume the
scratch-style `list(variables = list(tree = ...), param = ...)` objects often
used for large external run collections. Use `weights = "ic"` for comparable
Bifrost model-search outputs when model support should affect the summary; use
`weights = "equal"` for posterior samples where each retained draw should
contribute equally. When posterior trees share topology but branch lengths vary,
use `check = "topology"` with an explicit target tree. The target can be any
summary, consensus, chronogram, MCC, maximum-likelihood, or otherwise curated
tree with the same topology and tip labels as the inputs; the summary is plotted
on that target's branch lengths while rates are matched by descendant clades.

```{r multi_run_rate_map, eval=FALSE}
# Example: summarize a set of completed Bifrost searches
jaws_rate_map <- rateMap(
  jaws_search_runs,
  res = 100,
  plot = FALSE,
  weights = "ic",
...
# For very large run lists, subsample explicitly before summarizing.
set.seed(1)
idx <- sample(seq_along(jaws_search_runs), size = 1000)
jaws_rate_map_sub <- rateMap(
  jaws_search_runs[idx],
  res = 100,
  workers = 8,
...
# If you want one value per edge rather than depth-grid intervals:
jaws_branch_rates <- rateMap(
  jaws_search_runs,
  summary = "branch",
  weights = "ic",
  plot = FALSE
...
# Same topology but posterior branch lengths differ:
jaws_posterior_branch_rates <- rateMap(
  jaws_posterior_runs,
  target_tree = jaws_summary_tree,
  check = "topology",
  summary = "branch",
  weights = "equal",
  uncertainty = TRUE,
...
plotRateMap(jaws_posterior_branch_rates, value = "sd", type = "arc")
```
