...
Let's explore the choices made in this function call:
  ...
  * `num_cores = 6`: To speed up the search, the function evaluates all candidate shift models in parallel. We've specified using 6 CPU cores for this task.
...
plot_ic_acceptance_matrix(matrix_data = 
                          ...
                          rate_limits = c(-700,300), baseline_ic = NULL)

#to set the baseline to the 'true' baseline, use the following

#plot_ic_acceptance_matrix(matrix_data = 
#                            jaws_search$model_fit_history$ic_acceptance_matrix, 
#                          rate_limits = c(-5000,400), baseline_ic = jaws_search$baseline_ic)
...
> Information-criterion (IC) trajectory across sequentially evaluated sub-models during the greedy shift search. Blue points and connecting lines indicate accepted shifts, red crosses indicate rejected shifts, and the black point marks the baseline IC (here, the first accepted shift). The grey line shows the per-step change in IC (right axis), with negative values indicating improvement. Labels highlight the baseline and minimum accepted IC values.
...
ratio_int <- if (is.finite(min_rate) && min_rate > 0) as.integer(round(max_rate / min_rate)) else NA_integer_

text((x0 + x1) / 2, y1 + vscale * (0.03 * yr), "Mean rate (slow \u2192 fast)", cex = 0.9)
