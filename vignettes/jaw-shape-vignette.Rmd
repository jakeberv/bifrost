...
knitr::include_graphics(
  normalizePath(
    "jaw-shape/Gogosardina_coatesi_jaw_customcrop.png",
    winslash = "/",
    mustWork = TRUE
  ),
  rel_path = FALSE
...
On first use, `bifrost_example_file()` downloads approximately 3 KB for the tree and 245 KB for the landmarks from the maintained GitHub copy of the publication's [Dryad dataset](https://datadryad.org/dataset/doi:10.5061/dryad.z08kprrqf). Later calls reuse the verified local cache. To explicitly check for an updated copy, run:

```{r refresh_jaw_tree, eval=FALSE}
# Refresh the jaw tree only when an updated copy is wanted.
bifrost_example_file("jaw-tree", refresh = TRUE)
```

```{r load_data, eval=FALSE}
# Resolve the tree and landmark data through the public example-data cache.
