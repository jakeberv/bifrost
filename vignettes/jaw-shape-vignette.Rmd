...
used for large external run collections. When posterior trees share topology but
branch lengths vary, use `check = "topology"` with an explicit target tree, such
as an MCC tree, so the summary is plotted on the target branch lengths while
rates are matched by descendant clades.
