...
This vignette has two goals. First, it shows how the Berv et al. (2026) post-hoc workflow maps onto reusable `bifrost` functions for fitting, summarizing, plotting, pGLS, PCA, and module diagnostics. Second, it recomputes the reported Supplementary Text 2.1 and Supplementary Figure 4 summaries from compact post-hoc artifacts, without requiring the full archived model object at runtime.
...
The full analysis object, `runs_with_posthoc.rds` in the Berv et al. (2026) Zenodo archive, stores complete `mvgls` fits and is too large for a package vignette. This part requests the trait matrix (`passerine-traits`, approximately 212 KB), sensitivity results (`passerine-sensitivity`, approximately 392 KB), and compact post-hoc summaries (`passerine-posthoc`, approximately 132 KB). First use downloads and verifies these three files; later calls reuse the verified cache. The compact post-hoc object retains all-run variance/correlation summaries, representative pGLS anchors, focal `min10.ic20.gic` post-hoc correlation matrices after the stricter tip-count filter used for Supplementary Figure 4, PCA inputs, trait labels, module definitions, and provenance. This explicit command checks for an update:

```{r refresh_passerine_posthoc, eval=FALSE}
# Refresh the compact post-hoc summaries only when an updated copy is wanted.
bifrost_example_file("passerine-posthoc", refresh = TRUE)
```

```{r load_part5_inputs}
# Resolve the compact inputs used throughout this part.
trait_path <- bifrost_example_file("passerine-traits")
posthoc_path <- bifrost_example_file("passerine-posthoc")
