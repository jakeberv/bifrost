...
This vignette computes that non-spatial shift-magnitude workflow from compact cached data. Parts 1-3 introduce the focal search, lineage-rate summaries, and transition table used here. We first show the single-run calculation on the focal `min10.ic20.gic` result, then load the compact sensitivity bundle to obtain the run-level shift-direction frequencies and pooled magnitude-density comparisons. The magnitude test compares `log(abs(rate_delta))` for increases and decreases with a Monte-Carlo two-sample KS test.

## Loading the compact focal result

This part requests the compact focal search (`passerine-search`, approximately
1.9 MB) and sensitivity results (`passerine-sensitivity`, approximately 392 KB).
First use downloads and verifies each file; later calls reuse the verified
cache. This explicit command checks for an updated sensitivity result:

```{r refresh_passerine_sensitivity, eval=FALSE}
# Refresh the compact sensitivity results only when an updated copy is wanted.
bifrost_example_file("passerine-sensitivity", refresh = TRUE)
```

Start with the compact focal result. This object represents the `min10.ic20.gic` search and keeps the final mapped tree plus fitted BMM regime rates.
...
sensitivity_path <- bifrost_example_file("passerine-sensitivity")
