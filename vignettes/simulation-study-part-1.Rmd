...
tables (`simulation-preview-tables`, approximately 5 KB). First use downloads
...
The table below uses cached summaries from the code shown here, so package builds do not rerun the longer simulation studies.
...
The cached preview reproduces manuscript-era summaries. Its true-negative denominator predates candidate-node-aware accounting, while fresh simulation runs use the current candidate-aware accounting. It is retained for reproducibility rather than silently recomputed.
