...
The public example-data resolver supplies the lightweight inputs used in each part on demand. The complete Berv et al. (2026) archive remains the citable paper snapshot and the source for proposal history, sensitivity searches, and candidate model fits.
...
Part 1 requests three files through `bifrost_example_file()`:

- passerine tree (`passerine-tree`), approximately 81 KB
- trait matrix (`passerine-traits`), approximately 212 KB
- compact focal search result (`passerine-search`), approximately 1.9 MB

First use downloads and verifies these files; later calls reuse the verified cache. The tree and trait matrix reproduce the `searchOptimalConfiguration()` setup shown below. The compact focal result preserves the fitted BMM regime-rate parameters, mapped SIMMAP trees, accepted shifts, regime VCVs, IC weights, and IC acceptance matrix used by `icTrajectory()`, but omits the full optimized `mvgls` object and individual candidate fits. Use the full Zenodo archive for proposal-level history, sensitivity searches, full fitted model objects, and proposal metadata. To explicitly check for an updated focal result, run:
...
```{r load_data}
# Resolve the tree and species-level matrix through the public cache.
...
The matrix orders the 12 skeletal traits first and `vertnet_mass` last. That layout makes the model formula explicit:
...
The current `bifrost` API returns a `bifrost_search` object. The examples below use the compact cached `bodyplan_search` object you loaded above; the same code applies to a result that `searchOptimalConfiguration()` creates or that you load from the complete archived focal run.
...
- The cached tree and trait matrix let you reproduce the focal `searchOptimalConfiguration()` setup.
- The compact `bifrost_search` object preserves the final optimized model, mapped tree, and IC trajectory needed for Parts 1-3, plus the single-run shift-magnitude example in Part 4.
