...
#' convert the fitted object into a reusable calibration template for null and
#' shift-recovery simulation studies.
#'
#' @param baseline_tree A rooted phylogenetic tree of class `phylo` (or coercible
#'   via [ape::as.phylo()]) whose unique, non-empty tip labels match the row
#'   names of `trait_data`.
...
#'   and match `baseline_tree$tip.label`. Column names may be omitted for matrix
#'   inputs; when present, they must be unique and non-empty.
#' @param formula Formula specification passed to [mvMORPH::mvgls()]. May be a
#'   single character string or a formula object. Indexed formulas that reference
#'   `trait_data` directly are supported (for example, `"trait_data ~ 1"` or
#'   `"trait_data[, 1:12] ~ trait_data[, 13]"`), as are named-column formulas
#'   such as `cbind(y1, y2) ~ size + grp`.
...
#'   a mix resolvable against `trait_data`. Because R coerces mixed atomic
#'   vectors to character, integer-like strings are treated as positions when
#'   they are not exact column names. Numeric positions must be whole numbers.
#'   For intercept-only workflows this defaults to all columns. For any workflow
#'   where the response is a subset of `trait_data`, including formulas such as
...
#'   \item the full empirical residual covariance and residual degrees of
#'         freedom used by the empirical Wishart generator, and
#'   \item summaries of the diagonal and off-diagonal elements
#'         (`variance_mean`, `variance_sd`, `covariance_mean`, `covariance_sd`)
#'         retained by the manuscript-reproduction generator.
#' }
#'
...
createSimulationTemplate <- function(baseline_tree,
  ...
  if (!ape::is.rooted(aligned_tree)) {
    stop("baseline_tree must be rooted.")
  }
  validateSimulationIdentifiers(aligned_tree$tip.label, "tree tip labels")
  validateSimulationIdentifiers(rownames(trait_data), "trait_data row names")
  if (!is.null(colnames(trait_data))) {
    validateSimulationIdentifiers(colnames(trait_data), "trait_data column names")
  }
  ...
  calibration_tree <- phytools::paintSubTree(
  ...
  residual_df <- global_model$dims$n - global_model$dims$rank
  residual_df <- as.integer(max(ncol(fitted_values) + 1L, residual_df))
